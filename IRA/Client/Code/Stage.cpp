...
HRESULT CStage::Ready_Layer_GameLogic(const _tchar * pLayerTag)
{
	...
	//NULL_CHECK_RETURN(pGameObject, E_FAIL);
	//FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Monster_2", pGameObject), E_FAIL);

	//pGameObject = CEvilSoul::Create(m_pGraphicDev);
	//NULL_CHECK_RETURN(pGameObject, E_FAIL);
	//FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Monster_3", pGameObject), E_FAIL);

	//pGameObject = CMutationEvilSoul::Create(m_pGraphicDev);
	//NULL_CHECK_RETURN(pGameObject, E_FAIL);
	//FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"Monster_4", pGameObject), E_FAIL);
	...
	//CTransform* pTransformCom = dynamic_cast<CTransform*>(Engine::Get_Component(L"Layer_GameLogic", dynamic_cast<CJar*>(pGameObject)->Get_ObjKey(), L"Proto_Transform", ID_DYNAMIC));
...
HRESULT CStage::Ready_Layer_UI(const _tchar * pLayerTag)
{
	...
	pGameObject = CBoss_Bar::Create(m_pGraphicDev);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	FAILED_CHECK_RETURN(pLayer->Add_GameObject(L"FrontUI_Boss_Bar", pGameObject), E_FAIL);
...
HRESULT CStage::Load_Dynamic_Object_Info(const _tchar* pPath)
{
	HANDLE hFile = CreateFile(pPath, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	...
	while (true)
	{
		...
		case BUSH1: case BUSH2: case BUSH3: case BUSH4: case BUSH5: case BUSH_FLOWER1: case BUSH_FLOWER2:
		{
			...
			FAILED_CHECK_RETURN(pLayer->Add_GameObject(dynamic_cast<CSpriteObj*>(pGameObject)->Get_ObjKey(), pGameObject), E_FAIL);
			Safe_Delete_Array<_tchar*>(pObjKey);
		...
		case THORN1: case THORN2: case THORN3: case THORN4: case THORN5: case THORN6:
		{
			...
			FAILED_CHECK_RETURN(pLayer->Add_GameObject(dynamic_cast<CDestructibleObj*>(pGameObject)->Get_ObjKey(), pGameObject), E_FAIL);
			Safe_Delete_Array<_tchar*>(pObjKey);
